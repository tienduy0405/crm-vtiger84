...
class Wards extends CRMEntity
{
    var $table_name = 'vtiger_wards';
    var $table_index = 'wardsid';

    var $customFieldTable = array('vtiger_Wardscf', 'wardsid');

    var $tab_name = array('vtiger_crmentity', 'vtiger_wards', 'vtiger_Wardscf');

    var $tab_name_index = array(
        'vtiger_crmentity' => 'crmid',
        'vtiger_wards' => 'wardsid',
        'vtiger_Wardscf' => 'wardsid');

    var $list_fields = array(
        /* Format: Field Label => Array(tablename, columnname) */
        // tablename should not have prefix 'vtiger_'
        'Assigned To' => array('crmentity', 'smownerid'),
        'Tên' => array('wards', 'ward_name'),
    ...
    var $search_fields = array(
        /* Format: Field Label => Array(tablename, columnname) */
        // tablename should not have prefix 'vtiger_'
        'Tên' => array('vtiger_wards', 'ward_name'),
    ...
    var $popup_fields = array('ward_name');

    // For Alphabetical search
    var $def_basicsearch_col = 'ward_name';

    // Column value to use on detail view record text display
    var $def_detailview_recname = 'ward_name';

    // Used when enabling/disabling the mandatory fields for the module.
    // Refers to vtiger_field.fieldname values.
    var $mandatory_fields = array('ward_name', 'assigned_user_id');

    var $default_order_by = 'ward_name';
    var $default_sort_order = 'ASC';

    public function __construct()
    {
        if (class_exists('LoggerManager')) {
            $this->log = LoggerManager::getLogger('Wards');
        } else {
            $this->log = Logger::getLogger('Wards');
        }
        $this->db = PearDatabase::getInstance();
        $this->column_fields = getColumnFields('Wards');
    }
    ...
    function vtlib_handler($moduleName, $event_type)
    {
        if ($event_type == 'module.postinstall') {
            ...
            self::updateAutoGenerateField($moduleName, 'WARDS');
    ...
    public static function updateAutoGenerateField($moduleName, $prefix = 'WARDS')
    {
        global $adb;
