...
class Wards_Uninstall_Action extends Vtiger_Action_Controller
{
    ...
    public function process(Vtiger_Request $request)
{
    ...
    // vtiger_links
    $table = 'vtiger_wards';
    $tablecf = 'vtiger_wardscf';
    $table_user_field = 'vtiger_wards_user_field';
    ...
    $tabId = getTabid('Wards');
    $adb->pquery("DELETE FROM `vtiger_relatedlists` WHERE tabid = ?", [$tabId]);
    $adb->pquery("DELETE FROM `vtiger_relatedlists` WHERE label = ?", ['Wards']);
    ...
    // remove directory
    $res_template = $this->delete_directory('layouts/v7/modules/Wards');
    $res_module = $this->delete_directory('modules/Wards');

    if (!$res_module) {
        echo '
                <div style="margin-bottom: 4px;" class="helpmessagebox">
                    <b style="color: red;">ERROR</b> Can not delete the folder <b>modules/Wards</b>. Please check permission and run script again.
                </div>
            ';
    } else {
        echo "<script>
                alert('Unintall Wards module successfully');
